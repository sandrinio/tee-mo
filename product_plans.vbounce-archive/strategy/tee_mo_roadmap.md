...
status: "In Execution — Releases 1–3 core delivered (S-01–S-09). Documents + Wiki pipeline shipped (S-10–S-11). EPIC-017 Phase A live (search_wiki). Next: S-12 planning — EPIC-018 Scheduled Automations + EPIC-008 Setup Wizard remaining work."
...
| ADR-027 | **Wiki as Primary Knowledge Path** | The agent uses `search_wiki` (BM25 full-text retrieval over `teemo_wiki_pages`) as the primary knowledge retrieval strategy. `read_document` (fetches raw `teemo_documents.cached_content` by UUID) is a secondary fallback for: (a) exact quotes or verbatim data the agent explicitly needs, (b) documents that haven't completed wiki ingest yet (`sync_status != 'synced'`), (c) user explicitly references a specific file by name or ID. The conversation-tier system prompt must describe both paths and when to prefer each. | Wiki pages are pre-chunked, pre-summarized concept/entity units — dramatically better retrieval signal than raw document text. The `read_document` fallback ensures zero coverage gap during the ingest window and for precision lookups where structured synthesis would lose fidelity. Removing `read_drive_file` (the old tool) from the agent in STORY-015-03 was gated on having `read_document` as a drop-in replacement. | Decided | 2026-04-13 |
...
| S-05 | D-02 | 2026-04-12 | **Workspace CRUD delivered.** 6/6 stories Done. See archive. | v0.5.0 |
| S-06 | D-03 | 2026-04-12 | **BYOK key management delivered.** EPIC-004 complete. See archive. | v0.6.0 |
| S-07 | D-03 | 2026-04-12 | **AI Agent + Slack event loop delivered.** EPIC-007 complete. See archive. | v0.7.0 |
| S-08 | D-04 | 2026-04-13 | **EPIC-006 Google Drive fully delivered.** 6/6 stories Done, 2 bounces (006-02 Arch: refresh-as-Bearer, 006-03 QA+Arch: py39 compat + decrypt mock). 120+ tests. Ships: drive_service (6 MIME types + 50K truncation), scan_service (AI descriptions via scan-tier models), Drive OAuth (connect/callback/status/disconnect), Knowledge CRUD (index/list/delete + 15-cap + BYOK gate + async lock), Picker token, agent `read_drive_file` tool with self-healing hash check, frontend workspace detail page (Drive section + Google Picker + knowledge file list + progress banner), WorkspaceCard link. E2E fixes: `drive.readonly` scope, column name corrections, Slack mrkdwn formatting, streaming responses via `run_stream()`, display name resolution. ~5% avg correction tax on code stories, 15% on E2E. 5 flashcards pending approval. | (untagged) |
| S-09 | D-05 Release 3: Demo Polish | 2026-04-13 | **EPIC-008 Workspace Setup Wizard fully delivered.** 5/5 stories Done (3 Full Bounce + 2 Fast Track), 1 QA bounce (008-04 mechanical: sonner dep, stub prop, catch block — TL fixed), ~3.3% avg correction tax. 86 new tests (122 total passing). Ships: persistent top nav (AppNav + sonner toasts replacing FlashBanner), 4-step guided setup wizard (Drive → Key → Files → Channels with skip button), channel binding UI (picker with bound indicators, Active/Pending chips, unbind confirm, 409 error), workspace card polish (channel chips, DM badge, setup indicators, grid layout, dashed empty state), design token cleanup (zero #E94560 remaining, all buttons use Button component). Post-delivery walkthrough fixes: real Picker in step 3, ChannelSection in step 4 + detail page, skip button, bound channel red dot indicators. Bonus features (user-requested): multi-user team membership via `teemo_slack_team_members` table (Slack OAuth = membership gate), owner-only cascade team delete, team name display from Slack OAuth. ~1.06M agent tokens across 14 subagent tasks. | (untagged) |
| S-10 | D-06 | 2026-04-13 | **EPIC-006 enhancement wave fully delivered.** 5/5 stories Done (all Fast Track), 0 QA/Arch bounces, 0% correction tax across all stories. 37 new tests (102 total for EPIC-006). Ships: markdown-aware extractors (pymupdf4llm PDF → MD, DOCX/XLSX → markdown tables — STORY-006-07), multimodal LLM fallback for scanned/image PDFs via scan-tier vision (STORY-006-08), workspace hard delete with cascade cleanup (STORY-006-09), cached_content column for cache-first `read_drive_file` reads (STORY-006-10), re-index endpoint + frontend re-index button with spinner (STORY-006-11). EPIC-006 is now 100% complete (all 11 stories shipped). Sprint archived to `product_plans/archive/sprints/sprint-10/` (TBD). | (untagged) |
| S-11 | D-06 | 2026-04-13–14 | **Documents redesign + full Karpathy wiki pipeline + structured logging delivered.** 10/10 stories Done, 0 QA/Arch bounces, 4.5% avg correction tax (all Enhancement — 2 subagent timeouts + 1 merge conflict, TL resolved directly). 159 new tests. Ships: `teemo_documents` (replaces `teemo_knowledge_index`, universal source-agnostic schema with SHA-256 hashing + sync_status state machine — STORY-015-01), route refactor (STORY-015-02), agent CRUD tools (`create_document`, `update_document`, `delete_document`, `read_document` — STORY-015-03), Drive sync cron (10-min hash-diff polling — STORY-015-05), frontend source badges (STORY-015-06), wiki tables `teemo_wiki_pages` + `teemo_wiki_log` + `read_wiki_page` tool (STORY-013-01), wiki ingest pipeline (doc → source-summary + concept + entity pages, AI judge ≥3.5 on 8 fixtures — STORY-013-02), wiki ingest cron (60s tick — STORY-013-03), wiki lint (structural checks — STORY-013-04), structured JSON logging with token redaction (STORY-016-01). EPIC-013, EPIC-015, EPIC-016 all 100% complete. ADR-027 recorded (wiki primary + read_document fallback). Sprint archived to `product_plans/archive/sprints/sprint-11/`. | (untagged) |
| Post-S-11 | EPIC-017 Phase A | 2026-04-14–15 | **Wiki retrieval (EPIC-017 Phase A) shipped as direct commits outside V-Bounce.** `search_wiki` agent tool (BM25 full-text search over `teemo_wiki_pages` using PostgreSQL `tsvector` + `websearch_to_tsquery`), knowledge-routing strategy added to system prompt (wiki-first, read_document fallback, cascade query guidance), anti-hallucination rule ("only cite what search_wiki returns"), Slack mrkdwn formatter reconciliation. Commits: `89f81d0`, `44f2e9a`, `ae86a6e`, `1e6213b`, `bcb32e4`. EPIC-017 Phases B–E (synthesis, cascade, curation UI) remain in backlog. | (untagged) |
| S-04 | D-01 Release 1: Foundation + Deploy + Slack Install | 2026-04-12 | **Real Slack OAuth install end-to-end delivered (EPIC-005 Phase A complete).** 6/6 stories Done, **0 QA bounces, 0 Architect bounces**, ~2.5% aggregate correction tax. 5 Fast Track + 1 Full Bounce (STORY-005A-04 L3 OAuth callback — the only Full Bounce, passed QA + Architect on first attempt with zero security findings). Ships: 4 new backend routes (`GET /api/slack/install` with signed-state JWT, `GET /api/slack/oauth/callback` with code exchange + AES-256-GCM encrypt + Supabase upsert + 5 redirect branches + 3 hard-fail branches, `GET /api/slack/teams` with explicit-column select for no-token-leakage, hardened `POST /api/slack/events` with HMAC-SHA256 v0 signature verification closing the S-03 TODO); `backend/app/core/encryption.py` (first codebase use of `cryptography.hazmat.primitives.ciphers.aead.AESGCM`, lazy key loading via `get_settings()`); `backend/app/core/slack.py` (single-import-point for slack_bolt, `request_verification_enabled=True`); `backend/app/api/deps.py::get_current_user_id_optional` (redirect-instead-of-401 for OAuth callbacks); frontend `/app` replaced placeholder welcome card with full Slack Teams page (empty state + team list + 5 flash banner variants + loading skeleton + inline error with retry, Install button is a real `<a href>` NOT onClick for full-page auth-cookie navigation); first-use `httpx.AsyncClient` pattern (hand-rolled `FakeAsyncClient` monkeypatch for tests); first-use Supabase `.upsert()` (omits `DEFAULT NOW()` columns to preserve original `installed_at` on re-install); RTL component test infrastructure (new `vitest.config.ts`, `test-setup.ts`, `jest-dom`/`jsdom` devDeps). Backend 44 → 73 tests (+29). Frontend 10 → 19 tests (+9). Total 92 passing. 7 flashcards recorded at sprint close (httpx module-level import, Supabase upsert DEFAULT NOW() exclusion, base64url padding, slack_bolt constructor arg, slack-events 400 body change, vitest+vite type conflict, RTL auto-cleanup globals). **1 P0 framework bug filed** for pre-S-05 fix: `.vbounce/scripts/complete_story.mjs` over-aggressive cell replacement corrupted the sprint plan on 5 consecutive runs — hand-patched each time, filed as improvement #2 with a suggested markdown-table-parser fix + golden-file test. **5 Phase B risks documented** in Architect audit for EPIC-005 hardening: TOCTOU on different-owner check, unvalidated oauth.v2.access response, coalesced missing-field warning, brittle httpx monkeypatch coupling, no retry/`resp.status_code` check before `.json()`. Merge commit `bf787ba` on main. | v0.4.0 |
...
| 2026-04-13 | **Sprint S-09 closed.** EPIC-008 fully delivered (5/5 stories). §7 Delivery Log updated. Frontmatter status updated. Sprint archived to `product_plans/archive/sprints/sprint-09/`. ~1.06M agent tokens, 86 new tests, 3.3% avg correction tax. Bonus: multi-user team membership + owner delete + team name display. 3 flashcards pending recording. | Team Lead (S-09 close) |
| 2026-04-15 | **Sprints S-10 + S-11 closed.** EPIC-006 enhancement wave complete (S-10: 5/5 stories, 37 tests, 0% tax). EPIC-013, EPIC-015, EPIC-016 all fully delivered (S-11: 10/10 stories, 159 tests, 4.5% avg tax). §7 Delivery Log updated with S-10, S-11, and EPIC-017 Phase A direct commits. ADR-027 added to §3 (wiki primary + read_document fallback). Frontmatter status updated. Sprint-11 archived to `product_plans/archive/sprints/sprint-11/`. Flashcard batch from S-11 pending user approval. | Team Lead (S-11 close) |
| 2026-04-12 | **Sprint S-04 closed and released as `v0.4.0`.** Merge commit `bf787ba` on main, pushed to origin → Coolify auto-deploy. §7 Delivery Log row appended. EPIC-005 Phase A fully delivered (Slack OAuth install end-to-end). Release 1 status: ~85% → ~92% delivered; only S-05 (EPIC-003 Slice B workspace CRUD) remains to close Release 1. 6/6 stories Done, 0 QA bounces, 0 Architect bounces, ~2.5% correction tax (matches S-03 baseline). 7 flashcards recorded after user batch approval (see FLASHCARDS.md "Slack OAuth (S-04)" and "Frontend Test Infrastructure (S-04)" sections). **P0 framework bug filed for pre-S-05 fix**: `.vbounce/scripts/complete_story.mjs` corrupted sprint plan table cells on 5 consecutive runs (improvement-suggestions.md #2). **5 Phase B risks documented** in Architect audit for EPIC-005 hardening post-hackathon. 1 process deviation recorded: Team Lead nearly bypassed subagents during a rate-limit block; user course-corrected; no code landed outside the subagent path. | Team Lead (S-04 close) |
