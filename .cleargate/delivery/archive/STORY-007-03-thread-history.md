...
> **Ported from V-Bounce (shipped).** Original: `product_plans.vbounce-archive/archive/sprints/sprint-07/STORY-007-03-thread-history.md`. Shipped in sprint S-07, carried forward during ClearGate migration 2026-04-24.
...
- **R3**: Speaker display names are resolved via `users.info` API call. Cache user lookups within a single call.
- **R4**: If `users.info` fails for a user, fall back to `"User <user_id>"` — never crash on name resolution.
- **R5**: The bot's own messages (matched by `bot_user_id` OR `bot_id` field) are labeled as assistant messages.
- **R6**: Messages are returned in chronological order (oldest first), excluding the very last message (the trigger).
- **R7**: Uses `slack_sdk.web.async_client.AsyncWebClient` with the decrypted bot token. Does NOT use Slack Bolt.
- **R8**: Module has NO FastAPI imports.
