...
> **Ported from V-Bounce (shipped).** Original: `product_plans.vbounce-archive/archive/sprints/sprint-06/STORY-004-02-provider-resolvers.md`. Shipped in sprint S-06, carried forward during ClearGate migration 2026-04-24.
...
| Item | Action | Source |
|------|--------|--------|
| `backend/app/core/keys.py` | **Copy + strip** | `new_app/backend/app/core/keys.py` → `get_workspace_key()`. **Strip**: instance-key fallback (no `chy_instance_provider_keys` in Tee-Mo), settings fallback path. **Change**: query `teemo_workspaces` by `workspace_id`. |
| `backend/app/services/provider_resolver.py` | **Copy + strip** | `new_app/backend/app/services/provider_resolver.py` → `resolve_provider_key()`. **Strip**: `scope`, `key_id` metadata return. **Simplify**: single return value (plaintext key string), raises ValueError on missing. |
