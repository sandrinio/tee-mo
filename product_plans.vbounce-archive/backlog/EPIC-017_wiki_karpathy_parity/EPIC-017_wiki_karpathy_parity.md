...
owner: "Solo dev"
priority: "P1 - High"
tags: ["backend", "agent", "wiki", "rag", "karpathy", "quality"]
target_date: "TBD"
depends_on: ["EPIC-013", "EPIC-015"]
---
...
2. **Knowledge doesn't compound.** When the agent answers a multi-source question (e.g. "compare V-Bounce to SDLC Analysis"), the synthesis disappears into chat history. Karpathy's rule: *"Valuable findings become new wiki pages rather than disappearing."* We lose every insight the agent produces.
...
4. **Missing page types.** Karpathy's pattern calls out **comparison** and **synthesis** pages as first-class citizens. Our enum has `source-summary`, `concept`, `entity` only. Comparison/synthesis content has no home.

5. **Wiki is agent-only, not human-curated.** Karpathy's design is a *co-evolution loop* — human reviews, re-titles, removes bad pages. Our wiki has no review surface; bad LLM output stays until re-ingest.

6. **Lint only catches structural issues.** Our lint finds orphans / stale / missing coverage / low-confidence pages. Karpathy's lint also catches **contradictions** and **stale claims** across sources — requires LLM, not pure DB queries.

### 1.2 The Solution

Bring Tee-Mo's wiki to **Karpathy parity** in five incremental steps:

1. **Replace prompt stuffing with retrieval.** Add `search_wiki(query)` agent tool. System prompt shows only the top-N recently-relevant pages + a compact slug catalog. Agent searches for specific topics instead of seeing everything.

2. **Synthesis page creation from queries.** When the agent produces a multi-source answer worth keeping, it can call `create_wiki_page(type='synthesis', ...)` to persist the insight. Future queries benefit.

3. **Re-ingest cascade.** When a source document changes, identify related existing pages via cross-refs and update them (not just the source's own pages).

4. **Add `comparison` and `synthesis` page types.** Expand the enum, update ingest prompts to detect comparison-worthy content, and let the agent author synthesis pages.

5. **Human curation surface + LLM lint.** Minimal dashboard view of wiki pages with approve/delete actions. Extend `lint_wiki` with an optional LLM pass for contradiction detection (opt-in; expensive).

### 1.3 Success Metrics (North Star)

- **No more hallucinated document titles** — agent references only real pages from retrieval results (verify via live eval: ask "what documents do you have" across 5+ test workspaces, 0 fabrications)
- **Wiki grows organically** — at least 20% of new wiki pages in steady state are `synthesis` pages (agent-authored from queries)
- **Prompt size stays bounded** — system prompt < 8K chars regardless of wiki size (vs current unbounded growth)
- **Re-ingest cascade works** — updating a source document modifies at least 3 related pages on average (measurable via `teemo_wiki_log`)
...
**Phase A — Retrieval (solves the hallucination)**
- [ ] `search_wiki(query: str, top_k: int = 10) → list[dict]` agent tool — returns top-N matching pages (slug, title, tldr, snippet)
- [ ] Search implementation: **start with BM25 (Postgres `tsvector` / full-text search)** — no pgvector dependency for v1
- [ ] Add `search_vector` generated column to `teemo_wiki_pages` (tsvector over title + tldr + content)
- [ ] GIN index on `search_vector` for fast BM25
- [ ] Replace full `## Wiki Index` in system prompt with: (a) compact slug list (slug + title only, no TLDR), (b) instruction "Use `search_wiki(query)` to find pages by topic"
- [ ] Per-workspace page count guard — fall back to full index below 15 pages (small workspaces don't need retrieval)

**Phase B — Synthesis page creation**
- [ ] Add `synthesis` and `comparison` to `teemo_wiki_pages.page_type` CHECK constraint (migration)
- [ ] `create_wiki_page(slug, title, content, tldr, page_type, related_slugs)` agent tool — lets agent author pages directly (agent-owned, not from ingest)
...
- [ ] When `ingest_document` runs on a changed source, identify related pages via existing `related_slugs` where target page's sources overlap
...
- [ ] Bounded fan-out: max 10 pages updated per source change (prevents runaway cascades)

**Phase D — Ingest prompt improvements**
- [ ] Update ingest prompt to detect comparison candidates: if two concepts in the source are contrasted, emit a `comparison` page
...
- **Vector embeddings / pgvector** — BM25 first; vector only if BM25 proves insufficient (flag for EPIC-018)
- **Automatic synthesis creation** — agent *may* create synthesis pages but must be prompted or asked; never auto-creates
...
| Synthesis page creation becomes a hallucination vector | 🟢 Low | 🟡 Medium | Only allow agent to synthesize from pages it has actually read in the same conversation. Require explicit user confirmation before create. |
| Human curation UI is never used | 🟢 Low | 🟢 Low | Keep UI minimal; ship without if solo-dev pushback. |
| LLM lint produces too many false positives | 🟡 Medium | 🟡 Medium | Ship as opt-in only. Iterate the lint prompt against the RAG_TESTING corpus. |
...
|---|---|---|---|---|
| STORY-017-01 | BM25 search_wiki tool + tsvector migration | L2 | A | 3-4h |
| STORY-017-02 | Compact system prompt — replace full index with slug catalog + search guidance | L1 | A | 1-2h |
...
- **Sprint S-13**: Phase C + D + E — cascade, prompt tuning, curation. ~10-15h.
