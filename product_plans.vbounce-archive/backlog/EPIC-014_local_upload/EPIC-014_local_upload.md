...
status: "Ready"
ambiguity: "🟢 Low"
context_source: "Charter §5.2, §6 / Roadmap §3 ADR-005, ADR-006, ADR-007, ADR-016 / EPIC-006 codebase"
release: "Release 3: Demo Polish"
owner: "Solo dev"
priority: "P1 - High"
tags: ["backend", "frontend", "knowledge-base", "file-upload"]
...
- OCR for scanned PDF uploads (multimodal fallback only available for Drive files where we have the raw bytes stored; uploads discard the raw bytes after extraction)
...
--    For Drive files: unique on (workspace_id, drive_file_id) WHERE drive_file_id IS NOT NULL
--    For uploads: unique on (workspace_id, original_filename) WHERE source = 'upload'
ALTER TABLE teemo_knowledge_index
...
- Tradeoff: uploaded files cannot be re-extracted with improved extractors (unlike Drive files which can be re-fetched). This is acceptable for v1.
...
- The tool checks: if `drive_file_id` matches → Drive file path. If `id` matches → return `cached_content`.
...
3. **STORY-014-03: Agent integration** (L1)
   ...
   - No Drive fetch for uploaded files — always cache-only
...
|------|------------|------------|
| **Large file OOM** — 10MB XLSX with many sheets could expand significantly in memory | Low | 10MB limit + 50K char truncation. openpyxl streams data, doesn't load all into memory at once. |
| **Encoding issues** — non-UTF-8 text files | Medium | Attempt UTF-8 decode first, fall back to latin-1. Reject files that can't be decoded. |
| **MIME type sniffing mismatch** — user renames .txt to .pdf | Low | Trust the file extension / Content-Type header from the browser. If extraction fails, return a clear error. |
| **Race condition on upload** — same as Drive indexing | Low | Reuse the existing per-workspace `asyncio.Lock` from knowledge routes. |
...
| **Scanned PDF upload** — OCR fallback requires raw bytes which we discard | Low | Return best-effort pymupdf4llm text. Multimodal fallback not available for uploads (raw bytes discarded). Could store raw bytes temporarily for the multimodal call before discarding — future enhancement. |
...
Feature: Local Document Upload
  ...
  Scenario: Upload a plain text file
    Given a workspace with BYOK key configured
    When the user uploads a .txt file
    Then the backend reads it as UTF-8 text
    And generates an AI description
    And stores it in the knowledge index
  ...
  Scenario: Content truncation warning
    Given a workspace with BYOK key configured
    When the user uploads a file with >50K chars of content
    Then the response includes a truncation warning
    And cached_content is truncated at 50K characters
```
...
**References:**
- Roadmap: [Tee-Mo Roadmap](../../strategy/tee_mo_roadmap.md)
