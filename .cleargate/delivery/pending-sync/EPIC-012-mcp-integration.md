...
| **Performance** | MCP servers connected per request (no pooling). SSE handshake adds latency — acceptable for V1. |
...
- `POST /api/workspaces/:id/mcp-servers/:name/test` — test connection
...
- Each connected MCP server rendered as a mini-card with name, URL, status badge, enable/disable toggle, Test button, Delete button
...
| MCP server returns too many tools (bloats system prompt) | Low | Pydantic AI handles tool registration; token cost increases. Monitor in practice. |
...
| MCP SSE handshake timeout | A: 5s, B: 10s, C: 15s | Affects test-connection UX | sandrinio | Proposed: 10s |
...
| Test-connection implementation | A: Full SSE handshake + tool list, B: Just HTTP HEAD to URL | A is more accurate but slower | sandrinio | Proposed: A |
