...
async def test_build_agent_registers_four_skill_tools(monkeypatch: Any) -> None:
    ...
    assert len(tools_arg) == 7, (
        f"Expected 7 tools, got {len(tools_arg)}. "
        "Required: load_skill, create_skill, update_skill, delete_skill, web_search, crawl_page, http_request"
    )
